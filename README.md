...
### Sync vs Async Memory (`memory_mode`)
...
resp, _ := client.Agents.Chat(ctx, sonzai.AgentChatParams{
    AgentID: "agent-id",
    ChatOptions: sonzai.ChatOptions{
        Messages: []sonzai.ChatMessage{{Role: "user", Content: "Do you remember my favorite food?"}},
        UserID:   "user-123",
...
Set `SkipContextBuild: true` to bypass the context engine entirely (for lightweight test paths).
